||| A service is a (possibly stateful) asynchronous convertion from
||| a `Request` to a `Response`.
|||
||| Stateful services need to process their requests sequentially,
||| therefore, they typically use a queue (or just a `SnocList`) to
||| register requests.
|||
||| TODO: This should go to idris2-async
module Network.SCGI.Service
